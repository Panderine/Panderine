        ...
        if "require-title" in self.exceptions:
            for req in self.exceptions['require-title']:
                if not req.search(title):
                    return True
        return False
    def isCatExcepted(self, text):
        """
        Iff one of the exceptions applies for the given cat, returns True.
        """
        if "categorias" in self.exceptions:
            for exc in self.exceptions['categorias']:
            ...
            try:
                ...
                if not page.canBeEdited():
                    pywikibot.output(u"You can't edit page %s"
                                     % page.title(asLink=True))
                    continue
                if self.isCatExcepted (original_text):
                    pywikibot.output(
                        u'Salteando %s porque la catergoria esta en la exceptions list.'
                        % page.aslink())
                    continue
...
def main(*args):
    ...
    exceptions = {
        ...
        'categorias': [], #en realidad aqui son "plantillas" que hacen que se ignore el articulo si existen.
